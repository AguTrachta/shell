...
// ANSI Colors
#define COLOR_RED "\033[1;31m"     // Color code for red, used in prompt and startup animation
#define COLOR_WHITE "\033[1;37m"   // Color code for white, used in prompt
#define COLOR_GREEN "\033[1;32m"   // Color code for green, used in startup animation
#define COLOR_YELLOW "\033[1;33m"  // Color code for yellow, used in startup animation
#define COLOR_BLUE "\033[1;34m"    // Color code for blue, used in startup animation
#define COLOR_MAGENTA "\033[1;35m" // Color code for magenta, used in startup animation
#define COLOR_CYAN "\033[1;36m"    // Color code for cyan, used in startup animation
...
#define NETWORK_RECONNECT_DELAY 1500000 // Delay for network reconnecting message
...
void init_shell()
{
    ...
    /* Se limpia la mascara de seniales, es decir, no se bloquea el resto mientras funciona esta
     * SA_RESTART -> si la señal interrumpe una llamada al sistema (por ejemplo, read), esta debe reiniciarse
     * automáticamente SA_NOCLDSTOP -> Evita que la shell reciba senial SIGCHLD cuando un proceso hijo se detiene, solo
     * se recibe cuando termina
     */
...
char* read_command(FILE* input_stream)
{
    ...
    else
    {
        ...
        snprintf(prompt, sizeof(prompt), "%s%s@%s%s: %s%s%s $ ", COLOR_RED, username, hostname, COLOR_RESET,
                 COLOR_WHITE, current_folder, COLOR_RESET);
...
int execute_command(char* command)
{
    ...
    if (strchr(command, '|'))
    {
         ...
         * Commands es un array de cadenas, cada token contiene un comando a ejecutar.
         */
...
{
    job_t* new_job = malloc(sizeof(job_t)); // Asignacion de memoria a nuevo trabajo
    if (!new_job)
    {
...
{
    /* Busca un trabajo en la lista enlazada usando el pid, eliminando el proceso al encontrarlo
     * y liberando la memoria asignada.
     */
    job_t* current = job_list;
    job_t* prev = NULL;
