...
#define JSON_ACCUMULATED_BUFFER_SIZE (BUFFER_SIZE * 2) // Buffer size for accumulated JSON data
...
#define PID_WAIT_TIME 1     // Sleep time in seconds to wait for the process to terminate
...
{
    // Verificar si ya hay un proceso de monitoreo activo leyendo el archivo de PID
    ...
    if (pid_file)
    {
        ...
        if (fscanf(pid_file, "%d", &existing_pid) == 1)
        {
            ...
            if (kill(existing_pid, 0) == 0)
            {
                ...
                sleep(PID_WAIT_TIME); // Esperar un momento para asegurar que el proceso termine
            }
        }
        fclose(pid_file);
    }
    ...
    {
        // Proceso hijo: redirigir stdout y stderr a /dev/null para evitar mostrar en pantalla
...
int cmd_status_monitor(const char* option)
{
    if (strcmp(option, "--help") == 0)
    {
        ...
        printf("  -n     Shows only network statistics (bandwidth, packet ratio)\n");
...
int cmd_help()
{
    ...
    printf("\n--- External Commands ---\n");
    printf("Any external command available on the system, such as 'ls', 'cat', 'grep', etc.\n");
    printf("For more details on external commands, use 'man [command]'.\n");

    printf("\n--- Using Pipes (|) ---\n");
    printf("Use the '|' operator to chain commands, allowing the output of one command to be the input of another.\n");
    printf("Example: ls | grep 'name'\n");

    printf("\n--- Input and Output Redirection ---\n");
    printf("Use '>' to redirect the output of a command to a file, and '<' to read from a file as input.\n");
    printf("Examples:\n");
    printf("  echo 'Hello' > file.txt    - Writes 'Hello' to file.txt.\n");
    printf("  cat < file.txt             - Reads and displays the contents of file.txt.\n");

    printf("\n--- Executing Command Scripts ---\n");
    printf("You can execute files with a series of commands by using: ./filename or sh filename.\n");
    printf("Additionally, you can pass a command file when starting the shell to execute it automatically:\n");
...
int cmd_cd(char** args) // args[1] es el directorio al que se quiere cambiar
{
    ...
    if (chdir(target_dir) != 0) // Cambia el directorio del proceso actual al que voy a pasar.
    {
        perror("cd");
        free(current_dir);
        return 1;
...
int cmd_echo(char** args)
{
    ...
    for (int i = 1; args[i] != NULL; i++)
    {
        if (args[i][0] == '$')
        {
            ...
            char* env_var = getenv(args[i] + 1); // Si el argumento es variable de entorno se imprime el contenido
            if (env_var != NULL)
            {
                printf("%s ", env_var);
            }
...
int execute_internal_command(char** args, char* input_file, char* output_file, int background)
{
    if (strcmp(args[0], "cd") == 0 || strcmp(args[0], "clear") == 0 || strcmp(args[0], "echo") == 0 ||
        strcmp(args[0], "quit") == 0 || strcmp(args[0], "help") == 0 || strcmp(args[0], "start_monitor") == 0 ||
        strcmp(args[0], "stop_monitor") == 0 ||
        strcmp(args[0], "status_monitor") == 0) // Verifica si el comando es interno
    {
        int saved_stdin = -1, saved_stdout = -1; // Descriptores de archivos originales
        int fd_in = -1, fd_out = -1;             // Descriptores de archivos (si se especifican)

        /* Manejar redirección de entrada
         * El uso de dup y dup2 me permiten redirigir STDIN o STDOUT a sus respectivos archivos
         * para la ejecución de un código como echo
         * EJ: echo "Hello, world!" > output.txt */
        if (input_file)
        {
            ...
            if (dup2(fd_in, STDIN_FILENO) ==
                -1) // Permite modificar el archivo temporalmente sin tener que cambiar el original
            {
        ...
        else if (strcmp(args[0], "echo") == 0)
        {
            if (background)
            {
                ...
                else if (pid == 0)
                {
                     ...
                     * Esto es necesario para que los comandos internos puedan responder correctamente a
                     * señales como SIGINT, SIGTSTP, etc.*/
                ...
                else
                {
                    ...
                    if (job_id != -1) // Se agregó el trabajo en segundo plano exitosamente
                    {
                        printf("[%d] %d\n", job_id, pid);
                    }
                    else
        ...
        else if (strcmp(args[0], "status_monitor") == 0)
        {
            ...
            const char* option = args[1] ? args[1] : ""; // Obtiene el argumento, si existe
            result = cmd_status_monitor(option);
        }

        // Restaurar los descriptores originales si STDIN o STDOUT fueron usados.
        if (!background) // Ya se restauraron en el proceso hijo si es en segundo plano
        {
            if (saved_stdin != -1)
            {
                dup2(saved_stdin, STDIN_FILENO);
...
int execute_external_command(char** args, int background, char* command_copy, char* input_file, char* output_file)
{
    pid_t pid;
    int status;
    ...
    else if (pid == 0)
    {
        ...
        /* Restaurar manejadores de señales a los valores por defecto
         * Esto es necesario para que los comandos externos puedan responder correctamente a
         * CTRL-C, CTRL-Z, etc*/
        signal(SIGINT, SIG_DFL);
        signal(SIGTSTP, SIG_DFL);
        signal(SIGQUIT, SIG_DFL);
        ...
        // Ejecutar el comando, execv y execvp reemplazan el proceso actual por el programa especificado
        // execv usa el comando con el directorio especificado, por ej: /usr/bin/ls
        // execvp usa el comando buscando el directorio en PATH, por ej: ls
        if (strchr(args[0], '/'))
        {
    ...
    else // Proceso padre
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (wpid == -1)
                {
                    /* Si el error es por interrupcion del sistema, como read, write, otro waitpid
                     * implica que esa senial interrumpio la operacion antes de que se complete, y
                     * el sistema puede asumir que el proceso hijo ya termino cuando no es asi, por eso se
                     * continua esperando
                     * */
                    if (errno == EINTR)
                    {
...
int execute_single_command(char* command)
{
    ...
    int internal_status = execute_internal_command(args, input_file, output_file, background);
    ...
    int status = execute_external_command(args, background, command, input_file, output_file);
...
int execute_piped_commands(char** commands, int num_commands)
{
    ...
    pid_t pid;
    int status;
    int fd[2]; // Array para los descriptores de archivos (f[0] leer, f[1] escribir)
    ...
    for (i = 0; i < num_commands; i++) // Crea varios procesos hijos
    {
        ...
        if (i < num_commands - 1)
        {
            /* Se crea pipe con pipe(fd) que conecta la salida del comando actual con la entrada del siguiente */
            if (pipe(fd) == -1)
            {
                perror("Shell: pipe");
                exit(EXIT_FAILURE);
